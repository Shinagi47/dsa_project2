...
vector<anime> heapifyByNumRatings(vector<anime>& animes, int heapSize, int root) {
...
vector<anime> heapifyByAvgScore(vector<anime>& animes, int heapSize, int root) {
    ...
    // if right child score is larger than the largest score
    if (right < heapSize && vecCopy[right].score > vecCopy[largest].score) largest = right; 
...
vector<anime> heapSortByNumRatings(vector<anime>& animes, int heapSize) {
    vector<anime> vecCopy = animes; // create a copy

    // build the heap
    for (int i = heapSize/2 - 1; i >= 0; i--) vecCopy = heapifyByNumRatings(vecCopy, heapSize, i);

    for (int i = heapSize - 1; i >= 0; i--) {
        swap(vecCopy[0], vecCopy[i]);
        vecCopy = heapifyByNumRatings(vecCopy, i, 0);
    }
...
vector<anime> heapSortByAvgScore(vector<anime>& animes, int heapSize) {
    ...
    // build the heap
    for (int i = heapSize/2 - 1; i >= 0; i--) vecCopy = heapifyByAvgScore(vecCopy, heapSize, i);

    for (int i = heapSize - 1; i >= 0; i--) {
        swap(vecCopy[0], vecCopy[i]);
        vecCopy = heapifyByAvgScore(vecCopy, i, 0);
    }
